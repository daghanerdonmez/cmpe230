/*
NOWHERE NOBODY ve NOTHING tokenlerini ekledim. Artık subject/item olarak gözükmeyecekler.
go_to, more_than ve less_than tokenlerini ayırdım. Aralarında birden fazla whitespace olsa da okunacaklar.
Sonda newline olma durumunu düzelttim: \n karakteri de TOKEN_END sayılıyor.
Türkçe karakter sorunu için wchar.h include edilmeli. isalpha fonksiyonunu iswalpha ile değiştirdim.
*/
...
// Function to get the next token. Sets the current token to the new one, pushes the input pointer forward.
void getNextToken() {
    ...
    if (*input == '\0' && *input == '\n') {
        ...
        input += 4;
    } else if (strncmp(input, "NOBODY", 6) == 0) {
        current_token = TOKEN_NOBODY;
        input += 6;
    } else if (strncmp(input, "NOWHERE", 7) == 0) {
        current_token = TOKEN_NOWHERE;
        input += 7;
    } else if (strncmp(input, "NOTHING", 7) == 0) {
        ...
        input += 3;
    } else if (strncmp(input, "less", 4) == 0) {
        current_token = TOKEN_LESS;
        input += 4;
    } else if (strncmp(input, "more", 4) == 0) {
        current_token = TOKEN_MORE;
        input += 4;
    } else if (strncmp(input, "than", 4) == 0) {
...
// Returns the next token, without changing the position of the input pointer.
int peekToken() {
    while (isspace(*input)) ++input;
    if (*input == '\0' && *input == '\n') {
        return TOKEN_END;
    ...
    if (Subjects(0) == 1) {
        ...
        if (peekToken() == TOKEN_GO) {
            ...
            if (peekToken() == TOKEN_TO) {
                getNextToken();
                sentences[sentenceNo].actions[actionNo].keyword = TOKEN_GO;
                return Location(0);
            } else {
                return 0;
            }
...
bool checkConditions(SentenceStruct sentence){
    for (int i = 0; sentence.conditions[i].keyword != 0; ++i) {
        ...
        }else if (sentence.conditions[conditionNo].keyword == TOKEN_HAS + TOKEN_LESS){
            return subjectsHasLessThanItems(sentence.conditions[conditionNo].subjectNames, sentence.conditions[conditionNo].items);
        }else if (sentence.conditions[conditionNo].keyword == TOKEN_HAS + TOKEN_MORE){
